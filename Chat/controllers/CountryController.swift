...
extension String {
    static func flag(for code : String) -> String {
        ...
        func regionalIndicatorSymbol(fir scalar : Unicode.Scalar) -> Unicode.Scalar{
        ...
        let indicatorSymbols = lowercasedCode.unicodeScalars.map({regionalIndicatorSymbol(fir: $0)})
...
extension CountryController : UITableViewDelegate , UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.textLabel?.text = "\(self.dupList[indexPath.row].flag!) \( self.dupList[indexPath.row].name!) \(self.dupList[indexPath.row].extensionCode ?? "--")"
//        print(cell?.textLabel?.text)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let cellName = tableView.dequeueReusableCell(withIdentifier: "cell")
        let cellCode = tableView.dequeueReusableCell(withIdentifier: "cell")
        cellName?.textLabel?.text = "\(self.dupList[indexPath.row].flag!) \( self.dupList[indexPath.row].name!)"
        cellCode?.textLabel?.text = "\(self.dupList[indexPath.row].extensionCode ?? "--")"
        let phoneController = storyboard?.instantiateViewController(withIdentifier: "PhoneController") as? PhoneController
        phoneController?.name = cellName?.textLabel?.text ?? "Egypt"
        phoneController?.code = "+\(cellCode?.textLabel?.text ?? "20")"
...
extension NSLocale {
    func extensionCode(countryCode : String?) -> String? {
        ...
        let countryDialingCode = prefixCodes[countryCode ?? "EG"] ?? nil
